...
function bookHtml(trip: Trip, layout: 'print' | 'web' = 'print') {
  ...
  </style><style>.cover.has-photo { position: relative; overflow: hidden; color: #fff; } .cover-image { position: absolute; inset: 0; width: 100%; max-height: none; height: 100%; object-fit: cover; filter: brightness(.68); } .cover.has-photo .cover-dates { color: #e2f0ef; } .cover-content { position: relative; z-index: 1; text-shadow: 0 1px 12px #10263099; }</style></head><body class="${layout}${process.platform === 'linux' ? ' linux' : ''}"><section class="cover${coverPhoto ? ' has-photo' : ''}">${coverPhoto ? `<img class="cover-image" src="${escape(coverPhoto.path)}" alt="" />` : ''}<div class="cover-content"><h1>${escape(trip.title || 'Untitled journey')}</h1><p class="cover-subtitle">${escape(trip.subtitle || 'A travel book by icySteps')}</p>${dates ? `<p class="cover-dates">${escape(dates)}</p>` : ''}</div></section>${pages}${lightbox}</body></html>`
}

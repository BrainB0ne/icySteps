...
function initialiseDatabase() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS trips (id TEXT PRIMARY KEY, title TEXT NOT NULL, subtitle TEXT NOT NULL DEFAULT '', start_date TEXT NOT NULL DEFAULT '', end_date TEXT NOT NULL DEFAULT '', cover_photo_id TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL);
...
function listTrips(): Trip[] {
  const trips = db.prepare('SELECT id, title, subtitle, start_date as startDate, end_date as endDate, cover_photo_id as coverPhotoId, created_at as createdAt FROM trips ORDER BY created_at DESC').all() as Trip[]
...
function bookHtml(trip: Trip, layout: 'print' | 'web' = 'print') {
  ...
  </style><style>.cover.has-photo { position: relative; overflow: hidden; color: #fff; } .cover-image { position: absolute; inset: 0; width: 100%; max-height: none; height: 100%; object-fit: cover; filter: brightness(.68); } .cover.has-photo .cover-dates { color: #e2f0ef; } .cover-content { position: relative; z-index: 1; text-shadow: 0 1px 12px #10263099; } .cover-credit { margin: 16mm 0 0; color: #31596d; font: 8pt ui-sans-serif, sans-serif; letter-spacing: .08em; } .cover.has-photo .cover-credit { color: #e2f0ef; } .contents { min-height: 273mm; break-after: page; } .contents-frame { min-height: 273mm; padding: 18mm; border: 1px solid #8aa8a9; background: #fff; } .contents-kicker { color: #527c87; font: 9pt ui-sans-serif, sans-serif; letter-spacing: .12em; text-transform: uppercase; } .contents h2 { margin: 9mm 0 12mm; } .contents ol { margin: 0; padding: 0; list-style: none; } .contents li { border-top: 1px solid #d7e2df; } .contents li:last-child { border-bottom: 1px solid #d7e2df; } .contents a { display: grid; grid-template-columns: 14mm minmax(0, 1fr) auto; gap: 5mm; align-items: baseline; padding: 5mm 0; color: inherit; text-decoration: none; } .contents-number, .contents-date { color: #527c87; font: 9pt ui-sans-serif, sans-serif; letter-spacing: .08em; } .contents-title { font-size: 15pt; line-height: 1.2; } .step { scroll-margin-top: 18px; } .web .contents { min-height: 0; margin-bottom: 18px; break-after: auto; } .web .contents-frame { min-height: 0; } .web .contents a:hover .contents-title { color: #2f7181; text-decoration: underline; } @media (max-width: 620px) { .contents-frame { padding: 28px; } .contents a { grid-template-columns: 9mm minmax(0, 1fr); } .contents-date { grid-column: 2; } }</style></head><body class="${layout}${process.platform === 'linux' ? ' linux' : ''}"><section class="cover${coverPhoto ? ' has-photo' : ''}">${coverPhoto ? `<img class="cover-image" src="${escape(coverPhoto.path)}" alt="" />` : ''}<div class="cover-content"><h1>${escape(trip.title || 'Untitled journey')}</h1><p class="cover-subtitle">${escape(trip.subtitle || 'A travel book by icySteps')}</p>${dates ? `<p class="cover-dates">${escape(dates)}</p>` : ''}<p class="cover-credit">Created with icySteps v${escape(appVersion)}</p></div></section>${contents}${pages}${lightbox}</body></html>`
}
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('trips:create', (_, title: string) => { const trip = { id: uuid(), title: title.trim() || 'Untitled journey', subtitle: '', startDate: '', endDate: '', coverPhotoId: '', createdAt: now(), steps: [] }; db.prepare('INSERT INTO trips (id, title, subtitle, start_date, end_date, cover_photo_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(trip.id, trip.title, '', '', '', '', trip.createdAt); return trip })
  ipcMain.handle('trips:save', (_, trip: Pick<Trip, 'id' | 'title' | 'subtitle' | 'startDate' | 'endDate' | 'coverPhotoId'>) => db.prepare('UPDATE trips SET title=?, subtitle=?, start_date=?, end_date=?, cover_photo_id=? WHERE id=?').run(trip.title, trip.subtitle, trip.startDate, trip.endDate, trip.coverPhotoId, trip.id))

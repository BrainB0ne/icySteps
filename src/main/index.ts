...
function bookHtml(trip: Trip, layout: 'print' | 'web' = 'print') {
  ...
  </style></head><body class="${layout}${process.platform === 'linux' ? ' linux' : ''}"><section class="cover"><h1>${escape(trip.title || 'Untitled journey')}</h1><p class="cover-subtitle">${escape(trip.subtitle || 'A travel book by icySteps')}</p>${dates ? `<p class="cover-dates">${escape(dates)}</p>` : ''}</section>${pages}${lightbox}</body></html>`
}

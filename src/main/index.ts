...
const photoUrl = (path: string) =>
  `icy-photo://${Buffer.from(path).toString('base64url')}`
...
function initialiseDatabase() {
  ...
  try {
    db.exec(
      "ALTER TABLE trips ADD COLUMN cover_photo_id TEXT NOT NULL DEFAULT ''",
    )
  } catch (error) {
    if (
  ...
  try {
    db.exec("ALTER TABLE trips ADD COLUMN theme TEXT NOT NULL DEFAULT 'azure'")
  } catch (error) {
    if (
      !(error instanceof Error) ||
      !error.message.includes('duplicate column name')
...
function listTrips(): Trip[] {
  const trips = db
    .prepare(
      'SELECT id, title, subtitle, start_date as startDate, end_date as endDate, theme, cover_photo_id as coverPhotoId, created_at as createdAt FROM trips ORDER BY created_at DESC',
    )
    .all() as Trip[]
  const stepsForTrip = db.prepare(
    'SELECT id, trip_id as tripId, title, body, place_name as placeName, occurred_at as occurredAt, sort_order as sortOrder FROM steps WHERE trip_id = ? ORDER BY sort_order',
  )
  const photosForStep = db.prepare(
    'SELECT id, step_id as stepId, file_name as fileName, file_path, caption, sort_order as sortOrder FROM photos WHERE step_id = ? ORDER BY sort_order',
  )
  return trips.map((trip) => ({
    ...trip,
    steps: (stepsForTrip.all(trip.id) as Step[]).map((step) => ({
      ...step,
      photos: (
        photosForStep.all(step.id) as Array<Photo & { file_path: string }>
      ).map(({ file_path, ...photo }) => ({
        ...photo,
        path: photoUrl(file_path),
      })),
    })),
  }))
}
...
function updateManagedPhotoPaths() {
  const photos = db
    .prepare('SELECT id, step_id as stepId, file_name as fileName FROM photos')
    .all() as Array<{ id: string; stepId: string; fileName: string }>
  const update = db.prepare('UPDATE photos SET file_path=? WHERE id=?')
  db.transaction(() =>
    photos.forEach((photo) =>
      update.run(join(dataDirectory, photo.stepId, photo.fileName), photo.id),
    ),
  )()
}
...
function bookHtml(trip: Trip, layout: 'print' | 'web' = 'print') {
  ...
  const coverPhoto = trip.steps
    .flatMap((step) => step.photos)
    .find((photo) => photo.id === trip.coverPhotoId)
  const appVersion = app.getVersion()
  const renderPhoto = (photo: Photo, step: Step) => {
    const image = `<img src="${escape(photo.path)}" alt="${escape(photo.caption || step.title)}" />`
    return layout === 'web'
      ? `<button class="photo-button" type="button" data-full-photo="${escape(photo.path)}" data-full-alt="${escape(photo.caption || step.title)}" aria-label="Open ${escape(photo.caption || step.title || 'photo')} larger">${image}</button>`
      : image
  }
  const pages = trip.steps
    .map(
      (step, index) => `
    <article class="step" id="step-${step.id}"><div class="step-frame">
      <div class="chapter-number">Chapter ${String(index + 1).padStart(2, '0')}</div>
    ...
    for (const photo of step.photos) {
      ...
      try {
        await sharp(source)
          ...
          .toFile(join(imageDirectory, fileName))
        photo.fileName = fileName
      } catch {
        await copyFile(source, join(imageDirectory, photo.fileName))
      }
      photo.path = imageUrl(photo.fileName)
      onPhotoProcessed?.(++current, total)
    }
  }
  return linked
}

async function linkedBookHtml(
  trip: Trip,
  imageDirectory: string,
  ...
  const linked = await optimizedTrip(
    trip,
    imageDirectory,
    (fileName) =>
      `${encodeURIComponent(imageDirectoryName)}/${encodeURIComponent(fileName)}`,
    onPhotoProcessed,
  )
  return bookHtml(linked, 'web')
}

async function zipBookHtml(
  trip: Trip,
  outputPath: string,
  bookName: string,
  onPhotoProcessed?: (current: number, total: number) => void,
  ...
  const temporaryDirectory = await mkdtemp(
    join(app.getPath('temp'), 'icysteps-export-'),
  )
  ...
  try {
    ...
    for (const step of linked.steps) {
      for (const photo of step.photos) {
        archive.file(join(temporaryDirectory, photo.fileName), {
          name: `${imageDirectoryName}/${photo.fileName}`,
        })
      }
    }
    onPackaging?.()
...
async function createBackupArchive(outputPath: string) {
  ...
  archive.append(
    JSON.stringify(
      { format: 'icySteps backup', version: 1, createdAt: now() },
      null,
      2,
    ),
    { name: 'backup.json' },
...
function extractBackupArchive(backupPath: string, stagingDirectory: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    yauzl.open(
      ...
      (error, zipfile) => {
        if (error || !zipfile)
          return fail(error ?? new Error('Could not open backup archive.'))
        archive = zipfile
        zipfile.on('error', fail)
        zipfile.on('entry', (entry) => {
          const invalidName = yauzl.validateFileName(entry.fileName)
          const isAllowed =
            entry.fileName === 'backup.json' ||
            entry.fileName === 'icysteps.sqlite' ||
            entry.fileName.startsWith('projects/')
          if (invalidName || !isAllowed)
            return fail(new Error('Backup archive contains an invalid file.'))
          ...
          zipfile.openReadStream(entry, (streamError, stream) => {
            if (streamError || !stream)
              return fail(
                streamError ?? new Error('Could not read backup archive.'),
              )
...
async function restoreBackupArchive(backupPath: string) {
  ...
  try {
    ...
    const manifest = JSON.parse(
      await readFile(join(stagingDirectory, 'backup.json'), 'utf8'),
    ) as { format?: string; version?: number }
    if (manifest.format !== 'icySteps backup' || manifest.version !== 1)
      throw new Error('This is not a supported icySteps backup.')
...
app.whenReady().then(async () => {
  ...
  protocol.handle('icy-photo', (request) =>
    net.fetch(
      pathToFileURL(
        Buffer.from(new URL(request.url).hostname, 'base64url').toString(),
      ).toString(),
    ),
  )
  initialiseDatabase()
  ...
  ipcMain.handle('backup:create', async () => {
    const output = await dialog.showSaveDialog(mainWindow, {
      ...
      filters: [{ name: 'icySteps backup', extensions: [backupExtension] }],
    })
    if (output.canceled || !output.filePath) return null
    await createBackupArchive(output.filePath)
    return output.filePath
  })
  ipcMain.handle('backup:restore', async () => {
    const selected = await dialog.showOpenDialog(mainWindow, {
      title: 'Restore icySteps backup',
      properties: ['openFile'],
      filters: [{ name: 'icySteps backup', extensions: [backupExtension] }],
    })
    ...
    const answer = await dialog.showMessageBox(mainWindow, {
      ...
      defaultId: 0,
      cancelId: 0,
      message: 'Replace all current icySteps data?',
      detail:
        'This replaces every journey, chapter, and managed photo on this computer with the selected backup. This cannot be undone.',
    })
    if (answer.response !== 1) return false
    await restoreBackupArchive(selected.filePaths[0])
    return true
  })
  ipcMain.handle('trips:create', (_, title: string) => {
    const trip = {
      id: uuid(),
      ...
      startDate: '',
      endDate: '',
      theme: 'azure' as ThemeId,
      coverPhotoId: '',
      createdAt: now(),
      steps: [],
    }
    db.prepare(
      'INSERT INTO trips (id, title, subtitle, start_date, end_date, theme, cover_photo_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    ).run(trip.id, trip.title, '', '', '', trip.theme, '', trip.createdAt)
    return trip
  })
  ipcMain.handle(
    'trips:save',
    (
      ...
      db
        .prepare(
          'UPDATE trips SET title=?, subtitle=?, start_date=?, end_date=?, theme=?, cover_photo_id=? WHERE id=?',
        )
        .run(
          trip.title,
          trip.subtitle,
  ...
  ipcMain.handle('trips:delete', async (_, tripId: string) => {
    ...
    const photos = db
      ...
      .all(tripId) as Array<{ file_path: string }>
    db.transaction(() => {
      db.prepare(
        'DELETE FROM photos WHERE step_id IN (SELECT id FROM steps WHERE trip_id=?)',
      ).run(tripId)
      db.prepare('DELETE FROM steps WHERE trip_id=?').run(tripId)
      db.prepare('DELETE FROM trips WHERE id=?').run(tripId)
    })()
    await Promise.all(
      photos.map((photo) => rm(photo.file_path, { force: true })),
    )
    return true
  })
  ipcMain.handle('steps:create', (_, tripId: string) => {
    const order = (
      db
        .prepare('SELECT COUNT(*) as count FROM steps WHERE trip_id=?')
  ...
  ipcMain.handle('steps:save', (_, step: Step) =>
    db
      .prepare(
        'UPDATE steps SET title=?, body=?, place_name=?, occurred_at=? WHERE id=?',
      )
      .run(step.title, step.body, step.placeName, step.occurredAt, step.id),
  )
  ipcMain.handle('steps:reorder', (_, tripId: string, stepIds: string[]) =>
    db.transaction(() => {
      const update = db.prepare(
        'UPDATE steps SET sort_order=? WHERE id=? AND trip_id=?',
      )
      stepIds.forEach((id, index) => update.run(index, id, tripId))
    })(),
  )
  ipcMain.handle('steps:delete', async (_, stepId: string) => {
    const photos = db
      .prepare('SELECT file_path FROM photos WHERE step_id=?')
      .all(stepId) as Array<{ file_path: string }>
    db.transaction(() => {
      db.prepare(
        "UPDATE trips SET cover_photo_id='' WHERE cover_photo_id IN (SELECT id FROM photos WHERE step_id=?)",
      ).run(stepId)
      db.prepare('DELETE FROM photos WHERE step_id=?').run(stepId)
  ...
  ipcMain.handle('photos:import', async (_, stepId: string) => {
    ...
    const target = join(dataDirectory, stepId)
    await mkdir(target, { recursive: true })
    const start = (
      db
        .prepare('SELECT COUNT(*) as count FROM photos WHERE step_id=?')
        .get(stepId) as { count: number }
    ).count
    return Promise.all(
      selected.filePaths.map(async (source, index) => {
        const id = uuid()
        const fileName = `${id}-${basename(source)}`
  ...
  ipcMain.handle('photos:reorder', (_, stepId: string, photoIds: string[]) =>
    db.transaction(() => {
      const update = db.prepare(
        'UPDATE photos SET sort_order=? WHERE id=? AND step_id=?',
      )
      photoIds.forEach((id, index) => update.run(index, id, stepId))
    })(),
  )
  ipcMain.handle('photos:delete', async (_, photoId: string) => {
    ...
    db.transaction(() => {
      db.prepare(
        "UPDATE trips SET cover_photo_id='' WHERE cover_photo_id=?",
      ).run(photoId)
      db.prepare('DELETE FROM photos WHERE id=?').run(photoId)
    })()
    await rm(photo.file_path, { force: true })
  })
  ipcMain.handle('photos:save-caption', (_, photoId: string, caption: string) =>
    db.prepare('UPDATE photos SET caption=? WHERE id=?').run(caption, photoId),
  )
  ipcMain.handle('export:pdf', async (_, trip: Trip) => {
    const output = await dialog.showSaveDialog(mainWindow, {
      defaultPath: `${trip.title || 'icySteps-book'}.pdf`,
      filters: [{ name: 'PDF', extensions: ['pdf'] }],
    })
    if (output.canceled || !output.filePath) return null
    const total = photoCount(trip) + 1
    reportExportProgress({
      kind: 'pdf',
      current: 0,
      total,
      message: 'Optimizing photos...',
    })
    const temporaryDirectory = await mkdtemp(
      join(app.getPath('temp'), 'icysteps-export-'),
    )
    try {
      const optimized = await optimizedTrip(
        trip,
        temporaryDirectory,
        (fileName) => photoUrl(join(temporaryDirectory, fileName)),
        (current) =>
          reportExportProgress({
            kind: 'pdf',
            current,
            total,
            message: `Optimizing photo ${current} of ${photoCount(trip)}`,
          }),
      )
      reportExportProgress({
        kind: 'pdf',
      ...
      try {
        await window.loadURL(
          `data:text/html;charset=utf-8,${encodeURIComponent(bookHtml(optimized))}`,
        )
        const pdf = await window.webContents.printToPDF({
          preferCSSPageSize: true,
          printBackground: true,
        })
        await writeFile(output.filePath, pdf)
      } finally {
        window.destroy()
      }
      return output.filePath
  ...
  ipcMain.handle('export:html', async (_, trip: Trip) => {
    const output = await dialog.showSaveDialog(mainWindow, {
      defaultPath: `${trip.title || 'icySteps-book'}.html`,
    ...
    const imageDirectoryName = `${basename(output.filePath, extname(output.filePath))}-images`
    const imageDirectory = join(dirname(output.filePath), imageDirectoryName)
    const html = await linkedBookHtml(
      trip,
      imageDirectory,
      imageDirectoryName,
      (current) =>
    ...
    await writeFile(output.filePath, html)
    return output.filePath
  })
  ipcMain.handle('export:zip', async (_, trip: Trip) => {
    const output = await dialog.showSaveDialog(mainWindow, {
      defaultPath: `${trip.title || 'icySteps-book'}.zip`,
    ...
    await zipBookHtml(
      trip,
      output.filePath,
      basename(output.filePath, extname(output.filePath)),
      (current) =>
        reportExportProgress({
          kind: 'zip',
          current,

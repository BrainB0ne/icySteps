...
export default function App() {
  ...
  const [backupAction, setBackupAction] = useState<'create' | 'restore' | null>(
    null,
  )
  ...
  const createBackup = async () => {
    ...
    try {
      ...
      setBackupNotice(
        output
          ? { type: 'success', message: 'Backup created successfully.' }
          : { type: 'info', message: 'Backup cancelled.' },
      )
    } catch (error) {
      console.error(error)
      setBackupNotice({
  ...
  const reorderSteps = async (sourceId: string, targetId: string) => {
    ...
    await window.icySteps.reorderSteps(
      trip.id,
  ...
  const currentStepNumber =
    trip.steps.findIndex((step) => step.id === selectedStep) + 1
  ...
  const reorderPhotos = async (sourceId: string, targetId: string) => {
    ...
    const reordered = photos.map((photo, sortOrder) => ({
      ...photo,
      sortOrder,
    }))
    setTrip((current) => ({
    ...
    await window.icySteps.reorderPhotos(
      currentStep.id,
  ...
  const exportBook = async (kind: ExportKind) => {
    ...
    try {
      const output =
        kind === 'pdf'
          ...
          : kind === 'html'
            ...
            : await window.icySteps.exportZip(trip)
      setExportNotice(
        output
          ? {
              type: 'success',
              message: `${kind.toUpperCase()} travel book created successfully.`,
            }
      ...
      setExportNotice({
        type: 'error',
        message: `Could not create the ${kind.toUpperCase()} travel book.`,
      })
    } finally {
      setExporting(null)
  ...
  if (!trip.id)
    return (
      <main
        className={
          platform === 'linux' ? 'empty linux theme-azure' : 'empty theme-azure'
        }
      >
        ...
        <button
          className="text-button empty-restore"
        ...
        {backupNotice && (
          <p className={`backup-notice ${backupNotice.type}`}>
  ...
  return (
    ...
    >
      <aside className="sidebar">
        ...
        {exportNotice && (
          <p className={`export-notice ${exportNotice.type}`}>
        ...
        <div className="backup-actions">
          <button
            ...
            disabled={backupAction !== null || exporting !== null}
          >
        ...
        <nav>
          {trip.steps.map((step, index) => (
            <button
              key={step.id}
              className={`${selectedStep === step.id ? 'step-link active' : 'step-link'}${draggedStepId === step.id ? ' dragging' : ''}`}
        ...
        <section className="trip-fields panel">
          ...
          <label className="cover-photo-picker">
            ...
            <select
              ...
              onChange={(event) =>
                updateTrip({ coverPhotoId: event.target.value })
              }
              onBlur={() => void persistTrip()}
            >
                ...
                {currentStep.photos.map((photo) => (
                  <figure
                    ...
                    onDrop={(event) => {
                      event.preventDefault()
                      if (draggedPhotoId)
                        void reorderPhotos(draggedPhotoId, photo.id)
        ...
        <div className="book">
            ...
            <div className="preview-cover-content">
              ...
              {version && (
                <small className="preview-credit">
                  Created with icySteps v{version}
                </small>
              )}
            </div>
          </div>
          {currentStep && (
            <div className="preview-page">
              <small className="preview-chapter">
                Chapter {String(currentStepNumber).padStart(2, '0')}
              </small>
              <small>

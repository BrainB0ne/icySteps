...
const blankTrip: Trip = { id: '', title: '', subtitle: '', startDate: '', endDate: '', createdAt: '', steps: [] }
const dateRange = (startDate: string, endDate: string) => [startDate, endDate].filter(Boolean).map((date) => new Intl.DateTimeFormat('en', { day: 'numeric', month: 'long', year: 'numeric' }).format(new Date(`${date}T00:00:00`))).join(' - ')
...
export default function App() {
  ...
  const updatePhotoCaption = (photoId: string, caption: string) => updateStep({ photos: currentStep?.photos.map((photo) => photo.id === photoId ? { ...photo, caption } : photo) ?? [] })
  ...
  return <div className={platform === 'linux' ? 'shell linux' : 'shell'}>
    ...
    <main className="workspace">
      ...
      {currentStep ? <section className="editor panel">
        <div className="editor-head"><span className="eyebrow">Chapter {String(currentStep.sortOrder + 1).padStart(2, '0')}</span><button className="danger" onClick={() => void removeStep()}>Delete</button></div>
        <input className="step-name" value={currentStep.title} onChange={(e) => updateStep({ title: e.target.value })} onBlur={() => void persistStep()} placeholder="Give this moment a title" />
    ...
    <aside className="preview"><div className="preview-label">LIVE BOOK PREVIEW</div><div className="book"><div className="book-cover"><small>icySteps presents</small><h2>{trip.title}</h2><p>{trip.subtitle || 'A travel book'}</p>{(trip.startDate || trip.endDate) && <span className="preview-dates">{dateRange(trip.startDate, trip.endDate)}</span>}</div>{currentStep && <div className="preview-page"><small>{currentStep.occurredAt || 'A moment'}{currentStep.placeName ? ` / ${currentStep.placeName}` : ''}</small><h3>{currentStep.title || 'Untitled moment'}</h3><p>{currentStep.body || 'Your story will appear here.'}</p>{currentStep.photos.length > 0 && <div className={currentStep.photos.length === 1 ? 'preview-photos single' : 'preview-photos'}>{currentStep.photos.map((photo) => <figure key={photo.id}><img src={photo.path} alt={photo.caption || currentStep.title} />{photo.caption && <figcaption>{photo.caption}</figcaption>}</figure>)}</div>}</div>}</div></aside>
  </div>
}

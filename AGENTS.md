...
- `src/shared/themes.ts` is the shared palette source. Keep its colors aligned with `src/renderer/src/themes.css`; a journey's theme must affect the editor, live preview, and `bookHtml()` exports, including dark page backgrounds and readable text.
- Photo, chapter, and journey deletion use inline confirmations in the renderer. Keep Cancel non-blocking; do not replace them with synchronous browser dialogs.
